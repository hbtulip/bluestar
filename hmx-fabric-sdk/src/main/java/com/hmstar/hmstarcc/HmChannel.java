...
class HmChannel {
    ...
    //private Logger log = LogManager.getLogger(HmChannel.class);
    private static Logger log = Logger.getLogger(HmChannel.class);
    
    private String channelName; // myChannel
    private HmOrg org;
    private Channel channel;

    void init(HmOrg org) throws TransactionException, InvalidArgumentException {
        this.org = org;
     ...
     * Peer加入频道
     */
    JSONObject joinPeer(HmPeer peer) throws InvalidArgumentException, ProposalException {
        Properties peerProperties = new Properties();
        if (org.openTLS()) {
